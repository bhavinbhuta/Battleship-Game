...
public class Carrier implements Ship {
		...
		if (validation == Constants.VALIDATION_PASS) {
			...
			if (validation == Constants.VALIDATION_POSITION_ERROR) {
				...
				if (validation == Constants.VALIDATION_ORIENTATION_ERROR) {
					System.err.println("Ship is placed diagonally, which is not allowed! Please re-enter");
				} else {
	 ...
	 * ship the ships and accordingly pass to the place ship method.
	 * 
	 * @param: x1
	 ...
	 * @return: integer (0, 1, 2) - based on input received
	 * 
	 */
	...
	/**
	 * This method checks for the orientation of the ship so as to cater with
